...
export type AudienceCounts = { all: number; byLocation: Record<string, number> };
...
export function AnnouncementForm({
  ...
  const estimated = scope === "all" ? counts.all : (counts.byLocation[locationId] ?? 0);
    ...
    <form action={action} className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
      <Card className="bg-panel" padding="md">
        <div className="grid gap-5">
          <Field label="Title">
            <TextField name="title" placeholder="Water interruption advisory" required />
          </Field>
          <Field label="Message" hint={`${body.length} characters`}>
            <TextAreaField
              ...
              placeholder="What do tenants need to know?"
              className="min-h-40"
              required
            />
          </Field>
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="Audience">
              <SelectField name="scope" value={scope} onChange={(event) => setScope(event.target.value as typeof scope)}>
            ...
            {scope === "location" ? (
              <Field label="Location">
                <SelectField name="locationId" value={locationId} onChange={(event) => setLocationId(event.target.value)}>
                  {locations.map((loc) => (
          ...
          <fieldset className="grid gap-3 rounded-card border border-border/50 p-4">
            ...
            {(["email", "sms"] as const).map((channel) => (
              <label key={channel} className="flex items-center gap-3 text-sm text-text-secondary">
                <input type="checkbox" name={channel} defaultChecked={channel === "email"} className="h-4 w-4 accent-[color:var(--color-accent)]" />
      ...
      <Card className="bg-[#e9f5f3]" padding="md">
        ...
        <p className="mt-1 text-sm text-text-muted">Only occupied units with a tenant receive announcements.</p>
